(function () {
	...
	function main() {
		...
		try {
			if (sessionId) {
				...
				console.warn('no session id available; skipping localStorage sync');
			}
		} catch (e) {
	...
	var noShuffling = true;
	function addUrlShuffling() {
		// New URL format uses username as first path segment: /<username>/<target...>
		const username = (location.pathname.slice(1).split('/').filter(Boolean)[0] || '');
		if (!username) {
			console.warn('cannot get username from url');
			return;
		}
		// Server-side shuffle dictionary not used for username-based routing. Keep shuffling disabled by default.
		const shuffleDict = null;
		...
		const newUrl = oldUrl; // replaceUrl(location.href, (url) => shuffler._shuffle(url));
		// if (oldUrl !== newUrl) {
		...
		hammerhead.utils.url.overrideGetProxyUrl(function (url, opts) {
			...
			return getProxyUrl(url, opts);
			// return replaceUrl(getProxyUrl(url, opts), (u) => shuffler._shuffle(u), true);
		});
		...
		window.overrideGetProxyUrl(
			(getProxyUrl$1) =>
				function (url, opts) {
					...
					// return replaceUrl(getProxyUrl$1(url, opts), (u) => shuffler._shuffle(u), true);
				}
		);

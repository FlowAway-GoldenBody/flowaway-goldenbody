...
window.protectedGlobals.deregisterAppInstance = function (app, instance) {
  if (!app || !instance) return;
  var lists = window.protectedGlobals.getAllAppInstanceArrays(app);
...
window.protectedGlobals.allocateFallbackRuntimeGoldenbodyId = function () {
  var now = Date.now();
...
window.protectedGlobals.applyAppWindowTagging = function  (app, rootElement) {
...
window.protectedGlobals.resolveApptoolsContext = function (explicitAppId) {
  ...
  var app = (window.protectedGlobals.apps || []).find(function (a) {
    return window.protectedGlobals.appMatchesIdentifier(a, resolvedAppId);
...
window.protectedGlobals.initAppTools = function () {
  ...
  existing.api.trackInstance = function (instance, appOrId) {
    ...
    var appState = window.protectedGlobals.ensureAppRuntimeState(app);
    var keys = window.protectedGlobals.getAppInstanceArrayKeys(app);
  ...
  existing.api.untrackInstance = function (instance, appOrId) {
    ...
    if (app) {
      window.protectedGlobals.deregisterAppInstance(app, instance);
    }
    return instance;
  };
  ...
  existing.api.createAppInstance = function (ops) {
    ...
    if (app && !instance.goldenbodyId) {
      ...
      var fallbackAllocated = window.protectedGlobals.allocateFallbackRuntimeGoldenbodyId();
  ...
  try {
    ...
    for (const appFolder of appFolders) {
      try {
        ...
        if (appData) {
          window.protectedGlobals.ensureAppRuntimeState(appData);
    ...
    window.protectedGlobals.startAppPolling();
  } catch (e) {
...
window.protectedGlobals.renderAppsGrid = async function () {
  ...
  for (const app of window.protectedGlobals.apps) {
    try {
      window.protectedGlobals.ensureAppRuntimeState(app);
      if (!app.icon) {
...
window.protectedGlobals.ensureFlowawayAppPollingLoaded = async function () {
  try {
...
window.protectedGlobals.ensureProcessRuntimeLoaded = async function () {
  try {
...
window.protectedGlobals.getAppInstances = function (app) {
  if (!app) return [];
  try {
    return window.protectedGlobals.getPrimaryAppInstanceArray(app);
  } catch (e) {
    return [];
  }
};
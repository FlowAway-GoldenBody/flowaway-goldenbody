...
window.protectedGlobals.notification = function(message, options) {
  ...
  let currentWindowXY = { x: 50, y: 50 };
  let cnt = -1;
  const dx = 50;
  const dy = 50;
  const l2dx = 50;
  let l2cnt = 0;
  window.protectedGlobals.resetWindowXY = function() {
    currentWindowXY = { x: 50, y: 50 };
    cnt = -1;
    l2cnt = 0;
  }
  window.protectedGlobals.getNextWindowXY = function() {
    cnt++;
    if (cnt >= 5) {
      cnt = 0;
      currentWindowXY.x += l2dx;
      l2cnt++;
    }
    if (l2cnt >= 5) {
      l2cnt = 0;

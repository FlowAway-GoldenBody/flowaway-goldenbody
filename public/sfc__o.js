// ==UserScript==
// @name         Userscript Normalizer
// @description  Polyfills GM_* APIs and unsafeWindow for normal browser context
// @version      1.0
// @match        *://*/*
// @grant        none
// ==/UserScript==
    'use strict';
console.log('a');
/* Userscript Normalizer — Enhanced polyfill
   Usage: Put this BEFORE the userscript you want to run.
   Optional config: window.__GM_POLYFILL_OPTIONS__ = { scriptName: "MyScript", corsProxy: "https://your-proxy.example.com/" }
*/
  'use strict';

  // --- CONFIG ---
  const DEFAULT_OPTIONS = {
    scriptName: 'Userscript',        // used for namespacing storage
    corsProxy: null,                 // e.g. "https://cors.myhost.com/" (must accept full URL path)
    menuPosition: { bottom: 10, right: 10 }
  };
  const opts = Object.assign({}, DEFAULT_OPTIONS, window.__GM_POLYFILL_OPTIONS__ || {});

  // Short helper
  const ns = (k) => `GM_POLY_${opts.scriptName}::${k}`;

  // --- unsafeWindow: provide a safer-ish proxy that references page window
  // Note: this is NOT the same as real userscript sandboxing. It's just an alias.
  ...
  if (typeof GM_info === 'undefined') {
    window.GM_info = {
      script: {
        ...
        version: '0.0.0',
        description: 'Enhanced userscript polyfill'
      },
      platform: 'plain-page'
    };
  }

  // --- Storage (namespaced, JSON, promise-like behavior similar to GM)
  ...
  if (typeof GM_getValue === 'undefined') {
    window.GM_getValue = function (key, defaultValue) {
      try {
        ...
        return parsed && parsed.hasOwnProperty('v') ? parsed.v : defaultValue;
      } catch (e) {
        console.error('GM_getValue parse failed', e);
        return defaultValue;
      }
    };
  }
    ...
    window.GM_listValues = function () {
      try {
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (k && k.startsWith(`GM_POLY_${opts.scriptName}::`)) {
            keys.push(k.substring(`GM_POLY_${opts.scriptName}::`.length));
          }
        }
        return keys;
      } catch (e) {
        console.error('GM_listValues failed', e);
        return [];
      }
    };
  }

  // --- Clipboard (returns a Promise)
  if (typeof GM_setClipboard === 'undefined') {
    window.GM_setClipboard = function (text) {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        return navigator.clipboard.writeText(String(text)).catch(e => {
  ...
  if (typeof GM_openInTab === 'undefined') {
    window.GM_openInTab = function (url, options) {
      options = options || {};
      const win = window.open(url, options.active === false ? '_blank' : '_blank');
      return win;
    };
  }

  // --- GM_addStyle returns the style element for later removal
  if (typeof GM_addStyle === 'undefined') {
    window.GM_addStyle = function (css) {
      const style = document.createElement('style');
      style.setAttribute('data-gm-polified', opts.scriptName);
  ...
  // --- GM_download (improved)
  if (typeof GM_download === 'undefined') {
    window.GM_download = function (details) {
      // details: {url, name, data, type}
      try {
        if (details.url) {
          // use anchor method to download URL
          const a = document.createElement('a');
          a.href = details.url;
          if (details.name) a.download = details.name;
          a.target = '_blank';
          document.body.appendChild(a);
          a.click();
          a.remove();
          return true;
        } else if (details.data !== undefined) {
          const blob = new Blob([details.data], { type: details.type || 'application/octet-stream' });
          ...
          throw new Error('GM_download needs url or data');
        }
      } catch (e) {
  ...
  // --- Menu commands: visible floating button with dropdown
  if (typeof GM_registerMenuCommand === 'undefined') {
    (function () {
      const containerId = `gm-poly-menu-${opts.scriptName.replace(/\s+/g, '-')}`;
      ...
      function ensureContainer() {
        ...
        container.style.bottom = (opts.menuPosition && opts.menuPosition.bottom ? opts.menuPosition.bottom : 10) + 'px';
        container.style.right = (opts.menuPosition && opts.menuPosition.right ? opts.menuPosition.right : 10) + 'px';
        ...
        btn.textContent = '≡';
        btn.title = `${opts.scriptName} menu`;
      ...
      window.GM_registerMenuCommand = function (name, fn) {
        ...
        const list = c.querySelector('#' + containerId + '-list');
        ...
        item.addEventListener('click', (ev) => {
          try { fn(); } catch (e) { console.error('Menu command error', e); }
          list.style.display = 'none';
          ev.stopPropagation();
        });
        item.addEventListener('mouseenter', () => item.style.background = 'rgba(0,0,0,0.03)');
  ...
  // --- GM_xmlhttpRequest (fetch-based with CORS proxy fallback option)
  if (typeof GM_xmlhttpRequest === 'undefined') {
    window.GM_xmlhttpRequest = function (details) {
      if (!details || !details.url) {
        throw new Error('GM_xmlhttpRequest requires details.url');
      }

      const method = (details.method || 'GET').toUpperCase();
      const headers = details.headers || {};
      const body = details.data !== undefined ? details.data : null;
      let mode = 'cors';

      // if user explicitly wants same-origin, set mode
      if (details.anonymous === true) { /* do nothing special */ }

      function doFetch(urlToFetch) {
        const fetchOptions = {
          method,
          headers,
          body: body,
        ...
        return fetch(urlToFetch, fetchOptions)
          .then(async (resp) => {
            ...
            const res = {
              ...
              responseHeaders: (() => {
                try {
                  const raw = [];
                  resp.headers.forEach((v, k) => raw.push(`${k}: ${v}`));
                  return raw.join('\r\n');
                } catch (e) {
                  return '';
                }
            ...
            if (details.onload) try { details.onload(res); } catch (e) { console.error(e); }
            return res;
          });
      }

      // First try direct fetch (may fail due to CORS)
      return doFetch(details.url).catch((err) => {
        console.warn('Direct fetch failed (CORS?),', err);
        // If a corsProxy is configured, try proxying
        if (opts.corsProxy) {
          // proxy expects the target URL appended or encoded; user must set a compatible proxy.
          let proxyUrl = opts.corsProxy;
          // allow proxies that accept ?url= or path
          if (proxyUrl.indexOf('?') === -1 && !proxyUrl.endsWith('/')) proxyUrl += '/';
          const proxied = proxyUrl.includes('?') ? (proxyUrl + encodeURIComponent(details.url)) : (proxyUrl + details.url);
          return doFetch(proxied).catch((err2) => {
            console.error('CORS proxy fetch also failed', err2);
            if (details.onerror) try { details.onerror(err2); } catch (e) { console.error(e); }
            throw err2;
          });
        } else {
          if (details.onerror) try { details.onerror(err); } catch (e) { console.error(e); }
          throw err;
        }
      });
    };
  ...
  // --- GM_listValues already implemented above
  ...
  // --- Final: expose a helper to quickly reveal what can't be done
  window.GM_polyfill_info = {
    can: {
      storageNamespaced: true,
      injectStyle: true,
      openTabs: true,
      basicXhr: true,
      download: true,
    ...
    cannot: {
      bypassCORS: opts.corsProxy ? `only via configured proxy (${opts.corsProxy})` : 'cannot bypass CORS without a proxy or extension',
      realSandboxing: 'page-level script cannot emulate extension-level sandboxing',
      privilegedApis: 'no access to browser-level elevated privileges (e.g., GM.getTabs, real GM_registerMenuCommand native menu)'
    },
    optionsUsed: opts
  };

  // small indicator in console
  console.info(`GM polyfill loaded for script "${opts.scriptName}". See window.GM_polyfill_info for details.`);
    console.log("Userscript Normalizer loaded: GM_* APIs patched.");
